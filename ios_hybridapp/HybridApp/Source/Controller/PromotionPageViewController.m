//
//  PromotionPageViewController.m
//  Potato
//
//  Created by lzhang@juicyshare.cc on 2017/10/27.
...
@interface PromotionPageViewController ()
/** 5s的定时器 */
...
@implementation PromotionPageViewController
...
- (void)adViewClick {
    // 移除定时器
    [self removeTimer];
    [[ControllerFactory sharedInstance] setupHomeViewController];

    //[[ControllerFactory sharedInstance] openViewControllerWithView:self.navigationController ViewId:AD Object:nil Delegate:nil];
...
- (void)setupUI {
    ...
    if ([extensionName.lowercaseString isEqualToString:@"gif"]) {
        ...
        adView.image = [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:self.item.urlString];
    }
