...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if(_images){
        _toolBar.text = [NSString stringWithFormat:@"%zd/%zd",_currentIndex + 1,_images.count];
    }
    if(_imageUrls){
...
-(void)showLocalImageDatas:(NSArray <NSString *>*)images index:(NSInteger)index fromImageContainer:(UIView*)imageContainer
{
    _showNetImages = false;
    
    [self showImageDatas:images index:index container:imageContainer];
}

-(void)showImageDatas:(NSArray*)images index:(NSInteger)index container:(UIView *)container{
    //设置图片容器
    ...
    //更新显示
   // _toolBar.text = [NSString stringWithFormat:@"%zd/%zd",_currentIndex + 1,images.count];
    _toolBar.text = [NSString stringWithFormat:@"%zd/%zd",_currentIndex + 1,images.count];
    [_toolBar show];
